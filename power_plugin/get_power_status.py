...
def dell_power_status():
    try:
        cmd_state, cmd_result = commands.getstatusoutput('ipmitool  -I open sdr  | grep -E "^System Level|^Pwr"')
        if cmd_state == 0:
            power_list =  cmd_result.strip("\n").split("\n")
        else:
            return []
