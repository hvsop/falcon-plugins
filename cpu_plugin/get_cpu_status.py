...
def sugon_v3_cpu_status():
    try:
        cmd_state, cmd_result = commands.getstatusoutput("ipmitool sdr list | grep -E 'CPU[0-9]+_Stat'")
        if cmd_state == 0:
            cpu_status_list =  cmd_result.strip("\n").split("\n")
        else:
            return []
...
def h3c_cpu_status():
    try:
        cmd_state, cmd_result = commands.getstatusoutput("ipmitool sdr list | grep -E 'CPU_[0-9]+_Stat'")
        if cmd_state == 0:
            cpu_status_list =  cmd_result.strip("\n").split("\n")
        else:
            cmd_state, cmd_result = commands.getstatusoutput("ipmitool sdr list | grep -E 'CPU[0-9]+_Stat'")
