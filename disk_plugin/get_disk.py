    ...
    try:
        ...
        if cmd1_state == 0 and cmd2_state == 0:
            ...
            for item in range(list_number):
                k = l[item]
                v = status_list[item]
                if v == " Online" or v == " Unconfigured(good)" or v == " Hotspare":
                    v = 0
                else:
                    v = 1
                disk_dict = {}
...
def h3c_disk_status():
    try:
        cmd1_state,slot_number = commands.getstatusoutput("/usr/Arcconf/arcconf getconfig 1 pd| grep -E '( Programmed Max Speed)' -B 5 -A 2| grep 'Reported Channel,Device'| awk -F '[( )]' '{print \"Channel:Device-\"$(NF-1)}'")
        cmd2_state,disk_status = commands.getstatusoutput("/usr/Arcconf/arcconf getconfig 1 pd| grep -E '( Programmed Max Speed)' -B 5 -A 2| grep 'State'| awk -F ':' '{print $NF}'")
        if cmd1_state == 0 and cmd2_state == 0:
            slot_list = slot_number.split("\n")
            status_list = disk_status.split("\n")
            list_number = len(slot_list)
            l = []
            for i in range(list_number):
                temp = slot_list[i]
